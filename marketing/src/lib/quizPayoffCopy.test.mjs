...
describe("quiz payoff copy", () => {
  ...
  it("shows the app tour before the compact offer and keeps the lock-your-spot card", () => {
    ...
    assert.match(quizJs, /\$\$\{offerPrice\} · full rate \$\$\{fullPrice\}/);
    ...
    assert.doesNotMatch(quizJs, /capped at 50|50 spots|50 mamas/i);
    assert.doesNotMatch(quizJs, /Doors close/i);
    assert.doesNotMatch(quizJs, /Aug 27|Aug 31|August 31/);
    assert.doesNotMatch(quizJs, /lock your Aug 31 spot/i);
    assert.doesNotMatch(quizJs, /The whole group starts together/);
    assert.match(quizJs, /Callie builds every set of ranges by hand, in the order mamas lock in/);
    assert.match(quizJs, /Ready to lock your spot\?/);
