...
describe("message notification outbox", () => {
  ...
  it("retries a transient finish 401 and still schedules retry", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch")
      .mockResolvedValueOnce(new Response("unauthorized", { status: 401 }))
      .mockResolvedValueOnce(new Response(JSON.stringify([{ id: 10, status: "retry" }]), { status: 200 }));

    const retry = await finishNotificationJob(env, {
      id: 10,
      attempts: 2,
      claim_token: "10000000-0000-4000-8000-000000000010",
    }, {
      success: false,
      error: "push fanout incomplete",
    });
    ...
    expect(JSON.parse(fetchMock.mock.calls[1][1].body)).toEqual({
      p_job_id: 10,
      p_claim_token: "10000000-0000-4000-8000-000000000010",
      p_success: false,
      p_error: "push fanout incomplete",
    });
  });
